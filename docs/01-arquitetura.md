...
**2026-08-02 — Redesign "cara de app" (aprovado por mockups).** O visual anterior gritava "gerado por IA": emojis como ícones, nenhuma fonte própria, botões/inputs copiados inline em ~15 arquivos, sem toast/skeleton. Decidido: evoluir a identidade (roxo-noite + rosa), **Phosphor** como iconografia oficial (fill = ativo, regular = inativo; único import em `componentes/ui/icones.tsx`), **Fraunces variável self-hosted** como fonte de voz (corpo continua system-ui, como app nativo), primitivas em `componentes/ui/` (`Botao`, `Campo`, `FolhaBase`, `ProvedorAvisos`/`useAviso`, `Esqueleto`, `EstadoVazio`, `ControleSegmentado`) e a assinatura visual do **ingresso perfurado** (sessão e código de convite). Consequência: nenhum componente escreve mais classes de botão/campo à mão, e erro usa `--color-erro` — rosa ficou reservado para afeto.

**2026-08-03 — Web Push próprio, sem FCM/OneSignal.** Para dois usuários, serviço externo de push é peso morto. O caminho inteiro fica no que já existe: trigger SQL → `notificar_par()` → `pg_net` → Edge Function `enviar-push` (web-push/VAPID) → service worker. Consequências: o SW passou de `generateSW` para `injectManifest` (`src/sw.ts` próprio, com o mesmo precache/CacheFirst de antes); segredos (URL da função + segredo do gatilho) no Vault, nunca em migration; `notificar_par` engole erros — push jamais derruba a ação que o disparou; o EXECUTE é revogado de `authenticated` para ninguém spammar o par.
